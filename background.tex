...
% TODO include diagram of NMR setup too?
...
If the oscillating transverse magnetic field is applied at the Larmor frequency of the spins
\[
...
\begin{align*}
    ...
    \right] \\
    % \boxed{\overline{H}^{(0)} &= 1/3 \sum_j \delta_j \left(I_z^{(j)} + I_y^{(j)} + I_x^{(j)} \right)}
    % TODO fix ^
\end{align*}
