...
\begin{align}\label{eq:nmr-ham}
    ...
    H_Z &= \sum_j \omega_0 I_z^{(j)} \\
    H_\text{CS} &= \sum_j \delta_i I_z^{(j)} \\
    H_D &= \sum_{j,k} d_{jk} \left( 3I_z^{(j)}I_z^{(k)} - \mathbf{I^{(j)}} \cdot \mathbf{I^{(k)}} \right) \\
    H_{\text{rf}}(t) &=  u_1(t) \sum_j \omega_1 I_x^{(j)}
\end{align}
The term $I_z^{(j)}$ is shorthand for a tensor product of operators, all identity except for the $j$th operator
\[
...
\[
U(t) = \exp \left( -it \sum_j \omega_0 I_z^{(j)} \right)
\]
...
% TODO describe rotating frame, go through the math
% TODO assume rotating frame going forward??

If an oscillating transverse magnetic field is applied at the Larmor frequency of the spins
\[
...
\begin{table}
    ...
    \begin{tabular}{c c c c}
        Time interval & Toggling frame propagator $U_{\text{rf}}$ & $\widetilde{I_z}^{(j)}$ & $\widetilde{I_z}^{(j)}\widetilde{I_z}^{(k)}$ \\
        \hline
        $[0, \tau]$ & $\identity$ & $I_z^{(j)}$ & $I_z^{(j)}I_z^{(k)}$ \\
        $[\tau, 2\tau]$ & $U_X$ & $I_z^{(j)}$ & $I_z^{(j)}I_z^{(k)}$ \\
        $[2\tau, 4\tau]$ & $U_{\overline{Y}} U_X$ & $I_z^{(j)}$ & $I_z^{(j)}I_z^{(k)}$ \\
        $[4\tau, 5\tau]$ & $U_X$ & $I_z^{(j)}$ & $I_z^{(j)}I_z^{(k)}$ \\
        $[5\tau, 6\tau]$ & $\identity$ & $I_z^{(j)}$ & $I_z^{(j)}I_z^{(k)}$ \\
    \end{tabular}
\end{table}
...
We see that WHH-4 is cyclic because $U_{\text{rf}}(6\tau) = \identity$. To determine the lowest-order term in the Magnus Expansion $\overline{H}^{(0)}$, we use the toggled terms from table~\ref{tab:WHH-4} in equation~\ref{eq:AHT-terms-0}.
\begin{align*}
...
% TODO continue with WHH-4 example
