...
\titlegraphic{
\includegraphics[height=.1\textheight]{Dartmouth.eps}
\hfill
...
\begin{frame}[plain]{Average Hamiltonian theory}
...
\begin{frame}[plain]{Average Hamiltonian theory (cont.)}

\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{AHT-HO-diagram.pdf}
...
\begin{itemize}
    \item WAHUHA 4-pulse sequence (\cite{PhysRevLett.20.180}), $H_\text{eff} = \textcolor[rgb]{0,0.69,0}{H_{\text{CS}}'}$
    
    \item CORY 48-pulse sequence (\cite{CORY1990205}) designed analytically using AHT to be robust to errors, $H_\text{eff} = 0$
\end{itemize}
...
CORY-48 designed for time suspension while being robust to rotation and offset errors
...
Implemented AlphaZero algorithm (\cite{Silver1140}, originally for Chess, Shogi, and Go).
...
\begin{align*}
    ...
    \left\{
    \left[\widetilde{H}_{\text{sys}}(t_1), \left[\widetilde{H}_{\text{sys}}(t_2), \widetilde{H}_{\text{sys}}(t_3)\right]\right] +
...
\caption{(a) Ideal pulse shape,  (b) phase transients. From \cite{1976ii}.}
\end{figure}
