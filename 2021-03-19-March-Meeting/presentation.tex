...
\begin{itemize}
    % TODO reference Linta's, Ethan's talks
...
\note{
Average Hamiltonian Theory (AHT) has been used extensively for Hamiltonian engineering. AHT lets you design pulse sequences that refocus (cancel) certain terms in the Hamiltonian.
...
% TODO replace ^ with more data from Linta
\end{figure}

\note{
WHH-4 designed with pen and paper

CORY-48 designed to refocus all interactions while being robust to rotation and offset errors

AHT is great tool, but it's limited to approximating the average Hamiltonian with lowest-order terms. Longer sequences further restrict computing HO terms.

But longer pulse sequences can perform better\dots
...
% \pause
% TODO either figure out how to include this or drop it
...
Implemented AlphaZero algorithm (\cite{Silver1140}, originally for Chess, Shogi, and Go). Many different possible RL approaches (\cite{peng2021deep}).
% TODO call out to potential for tailored pulse sequences for particular systems, and robust to certain things...
% TODO note Pai's presentation earlier in the week
...
Action space is delay, $\pm$X, $\pm$Y. Target unitary is identity (want no dynamics, refocus all interactions).
...
\begin{frame}{Errors in experimental NMR}

\begin{itemize}
    \item Rotation (``flip angle'') errors
    \begin{figure}
    \centering
    % TODO make/find graphic for rotation errors
    \scalebox{.6}{
    \input{rotation-error.tex}
    }
    \end{figure}
    \item Phase transient errors
    \begin{figure}
    \centering
    \includegraphics[width=.6\textwidth]{phase_transients_viz.png}

    (a) Ideal pulse shape,  (b) phase transients, from \cite{1976ii}.
    \end{figure}
    % \item Resonance offset errors
    % TODO mention ^?
\end{itemize}
...
% TODO move experimental error description here...

\end{frame}

\begin{frame}{Computational results: AlphaZero algorithm learns}

\movie[width=.8\textwidth]{}{2021-03-15-12-pulse-sequence-rewards.mp4}
% TODO put in video showing learning, increasing rewards...
...
% TODO update figures below with new data once I get it...
\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{rot_errors-unconstrained.pdf}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{phase_transients-unconstrained.pdf.pdf}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{rot_errors-AHT.pdf}
\end{figure}

\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{phase_transients-AHT.pdf}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{rot_errors-robust.pdf}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{phase_transients-robust.pdf}
\end{figure}
...
\begin{figure}
\centering
\includegraphics[width=.75\textwidth]{decay_plot.pdf}
\end{figure}
% TODO replace this with actual experimental results I have...
...
\begin{itemize}
    % TODO mention dipolar interactions, start with physics again...
...
\begin{itemize}
    \item Sensing (refocusing dipolar interactions, e.g. $H_\text{eff} = \beta \textcolor[rgb]{0,0.69,0}{H_{\text{CS}}}$)
    \item Quantum memory (refocusing all interactions, $H_\text{eff} = 0$)
